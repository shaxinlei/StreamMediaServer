...
void FlashMainStream::messageHandler(const string& name,AMFReader& message,FlashWriter& writer) {
	DEBUG("Enter FlashMainStream::messageHandler");
	//INFO("%%%%%",name);hdf
	if(name=="connect") {
		INFO("MainString handle RTMP Commabd message connect");
		...
		INFO("MainString handle RTMP Commabd message setPeerInfo");
		...
		INFO("MainString handle RTMP Commabd message createStream");
		UInt16 idStream(1);
		...
		DEBUG("Create FlashStream in FlashMainStream::maeeageHandle()");
		FlashStream* pStream(new FlashStream(idStream, invoker, peer));   //创建FlashStream
		...
		INFO("MainString handle RTMP Commabd message deleteStream");
		double streamId;
