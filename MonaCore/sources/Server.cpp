...
void ServerManager::run(Exception& ex) {
	do {
		waitHandle();
	} while (sleep(9000) != STOP);    //Server线程睡眠9s
}
...
bool Server::start(const Parameters& parameters) {
	...
	Exception ex;
	bool result;
	EXCEPTION_TO_LOG(result = Startable::start(ex, Startable::PRIORITY_HIGH), "Server");    //启动Server线程
	if (result)    //server线程启动成功
		TaskHandler::start();
	return result;
}

void Server::run(Exception& exc) {
	_countClients = 0;   //客户端连接数
...
#endif
		Exception exWarn;
		if (((SocketManager&)sockets).start(exWarn) && ((RelayServer&)relayer).start(exWarn)) {   //启动线程SocketManger和RelayServer
			if (exWarn)
				WARN(exWarn.error());

			_pSessions.reset(new Sessions());   //销毁由_pSessions控制的session对象，并拥有新的session对象的所有权
			//NOTE("load protocols");
			_protocols.load(*_pSessions);    //加载协议，打开端口

			onStart();
			
			if (!_manager.start(exWarn, Startable::PRIORITY_LOW))    //启动ServerManger线程
				ex = exWarn;
			else if (exWarn)
				WARN(exWarn.error());
			while (!ex && sleep() != STOP){
...
void Server::manage() {          
	//NOTE("Server::manage include protocol,session,socket");
	_protocols.manage();
	if (_pSessions)
		_pSessions->manage();  //实现对session的管理，如终止已经死掉的session
	if(clients.count() != _countClients)         
		INFO((_countClients=clients.count())," clients");
	relayer.manage();          //清理sockets
}
