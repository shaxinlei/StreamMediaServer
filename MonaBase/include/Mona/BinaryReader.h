...
public:
	...
	void			moveCurrent(int size) { _current += size; }
	...
	UInt32			position() const { return _current-_data; }    //当前的相对位置（相对于起始位置）
	void			next(UInt32 count = 1) { _current += (count > available() ? available() : count); }     //当前位置+1，即下一个位置
	void			reset(UInt32 position = 0) { _current = _data+(position > _size ? _size : position); }    //设定当前位置
	void			shrink(UInt32 rest);    //收缩缓冲区

	const UInt8*	current() const { return _current; }       //当前绝对位置
	UInt32			available() const { return _end-_current; }  //可读字节数
