...
TCPClient::TCPClient(const SocketManager& manager) : _disconnecting(false),_pBuffer(manager.poolBuffers), _socket(manager),
		...
		//NOTE("active TCPClint onReadable listener to receive data");
		receive(ex,available);}),
...
void TCPClient::receive(Exception& ex, UInt32 available) {
	...
	UInt32 size(_pBuffer->size());   //初始size=_pBuffer->size()  为0
	//DEBUG("***_pBuffer->size():", _pBuffer->size());
	_pBuffer->resize(size+available,true);   //available=1537,重新设定buffer大小为1537
	//DEBUG("***after _pBuffer->reSize():", _pBuffer->size());
	...
	//DEBUG("***afterresize(size+received,true):", _pBuffer->size());
	size = 0;
	...
	while(size < _pBuffer->size()) { // while everything is not consumed
		...
		size = OnData::raise<0xFFFFFFFF>(_pBuffer); // consumed  处理接受到的数据   ,size=已处理数据

		if (!size) // if no consumption
			return; // no release here
	}

	_pBuffer.release();   // 释放缓冲
}
