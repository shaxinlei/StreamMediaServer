...
namespace Mona {
	...
	public:
		...
		NET_SOCKET acceptConnection(Exception& ex, SocketAddress& address) {
			...
			NET_SOCKET sockfd;
			do {
				DEBUG("accept TCP connection");
				sockfd = ::accept(_sockfd, (sockaddr*)&addr, &addrSize);  // 此处接受TCP连接,将套接字信息返回给应用程序
		...
		bool connect(Exception& ex, const SocketAddress& address, bool allowBroadcast) {
			...
			//在连接前调用，用来接收windows的FD_CONNECT事件
#endif
			//请求与服务器端的连接，并发出第一个SYN分节
			INFO("client call function connect ");
		...
		bool bind(Exception& ex, const SocketAddress& address, bool reuseAddress) {
			...
			INFO("server call function bind");
		...
		//服务器端调用bind绑定ip和端口号，并且开始监听监听（加载RTMP、HTTP、RTSP协议时调用)
		bool bindWithListen(Exception& ex, const SocketAddress& address, bool reuseAddress, int backlog) {
			...
			INFO("server call function bind and listen");
			int rc = ::bind(_sockfd, address.addr(), address.size());
			if (rc != 0 || ::listen(_sockfd, backlog) != 0) {   //如果服务器端套接字端口绑定成功则调用listen监听套接字
				Net::SetException(ex, Net::LastError(), " (address=", address.toString(), ", reuseAddress=", reuseAddress, ", backlog=", backlog, ")");
				return false;
			}

			managed(ex); // warning   //向套接字注册WSAAsyncSelect模型
			return true;
		}
		...
		bool managed(Exception& ex) {
			...
			if (ex)
				return false;
			return (_pManagedSocket = manager.add(ex, _sockfd, *_pSocket)) != NULL;    //向套接字绑定WSAAsyncSelect模型
		}
