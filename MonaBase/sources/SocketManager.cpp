...
namespace Mona {
...
#if defined(_WIN32)
		//DEBUG("SocketManger::handle:",_currentEvent);
...
#endif
			//DEBUG("call  function Socket::onReadable then call modle function OnReadable::raise");
	...
	void SocketManager::run(Exception& exThread) {
		...
		_eventSystem = 0;
#if defined(_WIN32)     //如果代码在windows平台上编译
		WNDCLASSEX wc;
		memset(&wc, 0, sizeof(wc));
		wc.cbSize = sizeof(WNDCLASSEX);        //用来保存本结构体的所占字节数
		wc.lpfnWndProc = WndProc;                //回调函数指针，用以指向前面那个回调函数
		wc.lpszClassName = name;                 //用来指向窗口类名字符串的指针,  作为这个窗口类的标识
		wc.cbClsExtra = 0;
		wc.cbWndExtra = 0;
		RegisterClassEx(&wc);                   //向windows系统注册窗口类
		_eventSystem = CreateWindow(name, name, WS_EX_LEFT, 0, 0, 0, 0, HWND_MESSAGE, NULL, NULL, NULL);     //创建窗口
		if (_eventSystem == 0)
...
#if defined(_WIN32)
		MSG msg;                   //它的类型是MSG，也是一个已预定义好的结构体，用来保存窗口操作的各种消息信息。
		int result;
		while ((result = GetMessage(&msg, _eventSystem, 0, 0)) > 0) {     // 注册消息循环体
...
//			}
		...
		DestroyWindow(_eventSystem);                  //销毁窗口
		if (result < 0)
